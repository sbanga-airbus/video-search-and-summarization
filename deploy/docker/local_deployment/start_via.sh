...
# Reinstall matplotlib to fix corrupted _c_internal_utils C extension (gilstate_tss_set crash)
pip3 install --force-reinstall --no-deps matplotlib==3.9.4 -q >/dev/null 2>&1
...
start_via_server() {
    ...
    if [ "$MODE" = "release" ]; then
        ...
        # Patch via_health_eval.py to lazy-load matplotlib so it's not imported
        # at module level (causes gilstate_tss_set TSS crash on H200 with Python 3.12).
        HEALTH_EVAL="via-engine/via_health_eval.py"
    else
	    echo "Starting VIA server in development mode"
	    EXE="python3 -Wignore src/via_server.py"
        HEALTH_EVAL="src/via_health_eval.py"
    fi
    # Patch via_health_eval.py: lazy-load matplotlib (avoids gilstate_tss_set TSS crash).
    # Patch via_server.py: pre-warm Python TSS by starting a dummy thread at the very top
    # of the script, before any torch/CUDA C extensions are loaded, so Python's TSS slots
    # are established first and can't be taken by CUDA background C threads.
    python3 - "$HEALTH_EVAL" "${HEALTH_EVAL/via_health_eval/via_server}" <<'PYEOF'
import sys, re

# --- Patch 1: via_health_eval.py - lazy matplotlib imports ---
path = sys.argv[1]
text = open(path).read()
text = re.sub(r'^import matplotlib\.colors as mcolors$', '# patched: import matplotlib.colors as mcolors', text, flags=re.MULTILINE)
text = re.sub(r'^import matplotlib\.pyplot as plt$', '# patched: import matplotlib.pyplot as plt', text, flags=re.MULTILINE)
text = text.replace(
    'self.color_list = list(mcolors.TABLEAU_COLORS.values())',
    'import matplotlib.colors as mcolors\n        self.color_list = list(mcolors.TABLEAU_COLORS.values())'
)
text = re.sub(r'(\n( +))(plt\.figure\()', r'\1import matplotlib.pyplot as plt\n\2\3', text)
open(path, 'w').write(text)
print(f"Patched {path}")

# --- Patch 2: via_server.py - pre-warm Python TSS before torch loads ---
path2 = sys.argv[2]
text2 = open(path2).read()
# Inject at the very top of the file (after the shebang/license comments) a dummy thread
# that runs immediately to pre-register Python TSS slots before torch C extensions load.
tss_warmup = (
    "# TSS warmup: start and join a dummy thread before torch loads to pre-register\n"
    "# Python TSS slots, preventing gilstate_tss_set crash on H200 with CUDA 13.\n"
    "import threading as _tss_warmup_threading\n"
    "_tss_t = _tss_warmup_threading.Thread(target=lambda: None, daemon=True)\n"
    "_tss_t.start(); _tss_t.join()\n"
    "del _tss_t, _tss_warmup_threading\n"
)
# Find the first non-comment, non-blank line that's an actual import/code
# and inject the warmup just before it
lines = text2.split('\n')
insert_at = 0
for i, line in enumerate(lines):
    ...
    if stripped and not stripped.startswith('#'):
        insert_at = i
        break
lines.insert(insert_at, tss_warmup)
text2 = '\n'.join(lines)
